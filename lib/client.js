...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS_TEXT = [
			...
			".dsh-sg-panel{position:fixed;left:calc(var(--dsh-sidebar-width,280px) + 10px);bottom:58px;width:380px;",
		...
		function applyGrouping() {
			...
			Array.prototype.forEach.call(document.querySelectorAll(SEC_SEL), (sec) => {
				...
				const firstDone = {};
				rows.forEach((r) => {
					const gid = cfg.assign[sessNameOf(r)];
					if (!gid) return;
		...
		function enhanceWsMenu() {
			...
			Array.prototype.forEach.call(document.querySelectorAll('[role="menu"]'), (menu) => {
				if (menu.dataset.dshSgPatched === "1") return;
				...
				const target = items.filter((i) => (i.textContent || "").indexOf("新建文件夹") >= 0)[0];
				if (!target) return;
				target.setAttribute("data-dsh-sg-item", "1");
				// 这一项是从「删除工作区」克隆来的，className 被清洗过（实测为空）→
				// 拿不到菜单项样式。沿用兄弟项「重命名」的类名，做到与上下两项同款。
