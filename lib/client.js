...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function applyCovers() {
			...
			Array.prototype.forEach.call(document.querySelectorAll(ROW_SEL), (row) => {
				...
				if ((it && it.key) || previewing) {
					row.classList.add("dsh-ws-cover");
					if (!previewing && it && it.key) {
						row.style.setProperty("--dsh-ws-cover",
							'url("' + COVER_API.img + "?k=" + it.key + "&t=" + (it.updatedAt || 0) + '")');
					}
					if (hp !== 34) row.style.height = hp + "px";
		...
		function pushCoverPreview(nm, dataUrl) {
			...
			if (!row) return;
			row.setAttribute("data-dsh-preview", "1");
			row.classList.add("dsh-ws-cover");
			row.style.setProperty("--dsh-ws-cover", 'url("' + dataUrl + '")');
			const hp = Math.max(28, Math.min(120, Number(coverGlobal.rowHeight) || 34));
			if (hp !== 34) row.style.height = hp + "px";
			else row.style.removeProperty("height");
		}
		...
		function resetRowOrder(r) {
			r.classList.remove("dsh-grp", "dsh-grp-first", "dsh-grp-hide");
			r.removeAttribute("data-dsh-grp");
			r.style.removeProperty("--dsh-grp-color");
			// 会话行本身不参与排序：真正参与排序的是包着它的 wrapper（见下）
			const wrap = r.parentElement;
			if (wrap && wrap.dataset && wrap.dataset.dshSgOrder === "1") {
				wrap.style.removeProperty("order");
				delete wrap.dataset.dshSgOrder;
			}
			...
			if (box && box.dataset && box.dataset.dshSgFlex === "1") {
				...
				Array.prototype.forEach.call(box.children, (c) => {
					if (c.style && c.style.order) c.style.removeProperty("order");
				});
			}
		}

		function clearGrouping() {
			Array.prototype.forEach.call(document.querySelectorAll(SESS_SEL), resetRowOrder);
		}
		...
		function applyGrouping() {
			...
			Array.prototype.forEach.call(document.querySelectorAll(SEC_SEL), (sec) => {
				...
				rows.forEach(resetRowOrder);

				const cfg = cfgOf(wsNameOf(wsRow), false);
				if (!cfg || !cfg.groups.length) return;

				// 1) 期望顺序：按分组在管理面板里的次序把同组会话**聚到一起**（组内保持原生先后），
				//    没归组的排最后。这是修「会话一多、同组被拆散、后面的被并进上一个标头」的关键。
				const order = {};
				let seq = 0;
				cfg.groups.forEach((g) => {
					rows.forEach((r) => {
						const n = sessNameOf(r);
						if (cfg.assign[n] === g.id) order[n] = seq++;
					});
				});
				rows.forEach((r) => {
					const n = sessNameOf(r);
					if (!cfg.assign[n]) order[n] = seq++;
				});

				// 2) 纯样式重排：会话行各自被一个 wrapper 包着（实测：每个 wrapper 只有 1 个子节点），
				//    所以让**布局容器**（groupSection）变 flex-column，再给每个 **wrapper** 设 order。
				//    全程只写内联样式，一个节点都不移动（移动 React 的节点会让它下次 diff 时崩）。
				const firstWrap = rows.length ? rows[0].parentElement : null;
				const box = firstWrap ? firstWrap.parentElement : null;
				if (box) {
					if (box.dataset.dshSgFlex !== "1") {
						box.dataset.dshSgFlex = "1";
						box.style.display = "flex";
						box.style.flexDirection = "column";
					}
					Array.prototype.forEach.call(box.children, (c) => {
						// 注意：工作区行**不是** box 的直接子元素，而是被一层容器包着
						// （实测 groupSection 的直接子元素只有 1 个）。所以必须用 querySelector 判断，
						// 否则会把工作区行当成「附属元素」推到最后 —— 表现就是
						// 「A 工作区的会话被塞进了 B 工作区的列表里」。
						if (c === wsRow || (c.querySelector && c.querySelector(ROW_SEL))) { c.style.order = "-1"; return; }
						if (c.dataset && c.dataset.dshSgOrder === "1") return;       // 会话 wrapper，稍后设
						if (!c.querySelector || !c.querySelector(SESS_SEL)) c.style.order = "9999"; // 其他附属元素推到最后
					});
				}
				...
				rows.forEach((r) => {
					...
					const wrap = r.parentElement;
					if (wrap) {
						wrap.dataset.dshSgOrder = "1";
						wrap.style.order = String(order[name] === undefined ? 999 : order[name]);
					}
					const gid = cfg.assign[name];
					if (!gid) return;
					const g = cfg.groups.filter((x) => x.id === gid)[0];
					if (!g) return;
					r.classList.add("dsh-grp");
					r.style.setProperty("--dsh-grp-color", g.color || COLORS[0]);
					if (!firstDone[gid]) {
						firstDone[gid] = true;
						r.classList.add("dsh-grp-first");
						r.setAttribute("data-dsh-grp", (cfg.collapsed[gid] ? "▸ " : "▾ ") + g.name);
					} else if (cfg.collapsed[gid]) {
						r.classList.add("dsh-grp-hide");
					}
				});
			});
		}
