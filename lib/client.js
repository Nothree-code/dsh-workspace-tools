...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 每个菜单容器**只处理一次**。以前用的是「标记 + 检查我的项是否还在」，
		// 一旦 React 把我插的项删掉，条件就不成立 → 每轮 pass 重新插一次 →
		// 每次插入又触发 childList → observer 再跑 → 又插 …… 直接死循环，
		// 表现就是点工作区 / 拖动排序后页面卡死、只能刷新。
		// 用 WeakSet 记住处理过的容器，插过就不再碰（哪怕它被 React 删掉）。
		const wsMenuDone = new WeakSet();
		function enhanceWsMenu() {
			if (!toggles.groups) return;
			Array.prototype.forEach.call(document.querySelectorAll('[role="menu"]'), (menu) => {
				if (wsMenuDone.has(menu)) return;
				...
				// 永远**自己克隆一项**插进菜单，绝不去改造别人的菜单项。
				// 早先的版本会「优先把 folder-tree-sh 的『新建文件夹』就地改成会话分组」，
				// 但那一项是它自己的状态：改掉它的文字/图标后，它后续渲染可能出错甚至反复重试。
				// 这正好解释「同一个插件在干净环境正常、在装了一堆插件的正式环境卡顿」——
				// 干净环境里没有那一项，永远不会走到那条分支。
				// 克隆节点不属于 React 树，它不会去动它；真正危险的是往 React 的**列表容器**里插节点。
		...
		function apply(ctx) {
			...
			// 拖动排序期间**彻底放手**：DSH 拖拽时按元素的真实位置 / DOM 顺序计算「该插到谁前面」，
			// 而我们的分组重排用的是 CSS order —— 只改视觉顺序、DOM 顺序没变。两者一打架，
			// DSH 会反复重算并调整，我们的 observer 又跟着重算，页面直接卡死。
			// 所以：按住工作区行（可能是在拖它）期间，一律不碰 DOM；松手后再恢复。
			let holdingRow = false;
			const onPointerDown = (e) => {
				const row = e.target && e.target.closest ? e.target.closest(ROW_SEL) : null;
				if (row) holdingRow = true;
			};
			const onPointerUp = () => {
			...
			document.addEventListener("dragend", onPointerUp, true);
			document.addEventListener("drop", onPointerUp, true);
			window.addEventListener("blur", onPointerUp, true);

			const pass = () => {
				if (holdingRow) return;      // 正在按住工作区行 → 不动 DOM，把控制权完全交给 DSH
